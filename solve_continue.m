...
function [X,Solinfo,Sol] = ...
    solve_continue(x0,fun_residual,lam_s,lam_e,ds,H,delt,N,varargin)
...
nu = lam_s*(1:1:H)';
[APDFT.Gamma, APDFT.t] = APDFT_fun(delt, N, nu);
...
if Sopt.flag || isfield(Sopt,'init') && Sopt.init.flag
    ...
    [XP,~,iEx,output,J] = fsolve(@(X) extended_residual(X, ...
        Xref,zref,fun_residual,APDFT,Sopt),X0,Solopt);
    ...
    [XP,~,iEx,output,J] = fsolve(@(X) extended_residual([X;lam_s], ...
        Xref,zref,fun_residual,APDFT,Sopt),X0(1:end-1),Solopt);
    X0(1:end-1) = XP;
end
if iEx<1
...
while istep<=Sopt.stepmax
    ...
    if Sopt.flag
        ...
        [Xtmp,Rext,iEx,output,Jtmp] = ...
            ...
            X,Xref,zref,fun_residual,APDFT,Sopt),XP,Solopt);
    else
        % Solve extended nonlinear system of equations
        [Xtmp,Rext,iEx,output,Jtmp] = ...
            fsolve(@(X) extended_residual( ...
            [X;XP(end)],Xref,zref,fun_residual,APDFT,Sopt),XP(1:end-1),Solopt);
    end
    %% ACCEPT/REJECT POINT
...
function [Rext,dRextdX] = extended_residual(X,Xref,zref,...
    fun_residual,APDFT,Sopt)
%% Evaluation of the residual function and its derivative
switch Sopt.jac
    case {'full','on'}
        [R,dRdX] = feval(fun_residual,diag(Sopt.Dscale)*X,APDFT);
    case 'x'
        [R,dRdx] = feval(fun_residual,diag(Sopt.Dscale)*X,APDFT);
...
function J = finite_difference_jacobian(Hfuncname, x0, APDFT, Smyopt, varargin)
%========================================================================
% DESCRIPTION:
...
for ii = 1:length(x0)
    ...
    switch (Smyopt.ikeyfd)
        ...
        case (1)
            ...
            if (ii == 1)
                f0 = feval(Hfuncname,x0,APDFT,varargin{:});
            ...
            fp = feval(Hfuncname,xp,APDFT,varargin{:});
            J(:,ii) = (fp-f0)/dx;
        ...
        case (2)
            ...
            % vector
            fm = feval(Hfuncname,xm,APDFT,varargin{:});
